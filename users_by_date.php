<?php 
	...
	if(!empty($_GET['date_from']) or !empty($_GET['date_to']) or !empty($_GET['canned_date'])){	
		...
		$sql = mysqli_query($mysqli,"SELECT SQL_CALC_FOUND_ROWS * FROM users WHERE user_created BETWEEN UNIX_TIMESTAMP('$date_from') AND UNIX_TIMESTAMP('$date_to 23:59:59') ORDER BY user_id DESC LIMIT $record_from, $record_to");
	}
